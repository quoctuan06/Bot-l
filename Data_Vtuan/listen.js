        ...
        if (day != day_now) {
            ...
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    ...
                    api.sendMessage({
    body: checkttBody, attachment: (await axios.get((await axios.get(`https://nguyenmanh.name.vn/api/world/vietnam`)).data.url, {
                    responseType: 'stream'
                })).data
   ...
   if (event.type == "change_thread_image") api.sendMessage(`=== 『 ${global.config.BOTNAME} 』 ===\n━━━━━━━━━━━━━━━━\n${event.snippet}`, event.threadID);
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID) &&!ndhBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        adminBot.forEach(e => {
          api.sendMessage(`=== 『 DUYỆT BOX 』 ===\n━━━━━━━━━━━━━━━━\n ➜ Tên: ${threadName}\n ➜ ID: ${event.threadID}\n ➜ Đã yêu cầu được duyệt\n➜ Liên hệ: m.me/QuocTuanXxx\n━━━━━━━━━━━━━━━━\n===「${thu} || ${gio}」===`, e);
        })
        return api.sendMessage({
    body: `=== 『 REQUEST SUCCESS 』 ===\n━━━━━━━━━━━━━━━━\n ➜ Đã gửi yêu cầu đến ${admin.length} ADMINBOT\n━━━━━━━━━━━━━━━━\n===「${thu} || ${gio}」===`, attachment: (await axios.get((await axios.get(`https://nguyenmanh.name.vn/api/world/vietnam`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID, () => {
