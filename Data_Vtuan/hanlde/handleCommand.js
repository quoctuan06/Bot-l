module.exports = function ({ api, event, args, models, Users, Threads, Currencies }) {
  ...
  return api.sendMessage({body: "=== 『 CatEmpress 』 ===\n━━━━━━━━━━━━━━━━\n\n➜ Chỉ ADMINBOT mới được sử dụng bot trong chat riêng.", attachment: (await axios.get((await axios.get(`https://nguyenmanh.name.vn/api/world/vietnam`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID);
}    
    ...
    body: '=== 『 CatEmpress 』 ===\n━━━━━━━━━━━━━━━━\n➜ Chỉ ADMINBOT mới có thể sử dụng bot', attachment: (await axios.get((await axios.get(`https://nguyenmanh.name.vn/api/world/vietnam`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID);
    }
     if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) { 
          return api.sendMessage({body: '=== 『 CatEmpress 』 ===\n━━━━━━━━━━━━━━━━\n➜ Chỉ SUPPORTBOT mới có thể sử dụng bot', attachment: (await axios.get((await axios.get(`https://nguyenmanh.name.vn/api/world/vietnam`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID);
...
const dataAdbox = require('./../../modules/commands/cache/data.json');
  ...
  const timeNow = moment.tz("Asia/Ho_Chi_Minh").format("DD/MM/YYYY || HH:mm:ss");
	...
	var seconds = Math.floor(time % 60);
      const res = await axios.get(`https://nguyenmanh.name.vn/api/thathinh?type=boy`);
var tpk = res.data.result.data;
      const allicon = ["💞","💖","💗","💜","🌸","💗","💝","🎀","🌹","🍁","🎊","🌟","🍁"];
...
attachment: (await axios.get((await axios.get(`https://nguyenmanh.name.vn/api/world/vietnam`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID);
    ...
    if (command.config.hasPermssion > permssion) return api.sendMessage({body: `『 𝗟𝗘̣̂𝗡𝗛 𝗔𝗗𝗠𝗜𝗡/𝗤𝗧𝗩 』\n━━━━━━━━━━━━━━━━━━\n➜ 𝗕𝗮̣𝗻 𝗸𝗵𝗼̂𝗻𝗴 𝘁𝗵𝗲̂̉ 𝗱𝘂̀𝗻𝗴 𝗹𝗲̣̂𝗻𝗵 𝗻𝗮̀𝘆\n➜ 𝗟𝗲̣̂𝗻𝗵 ${command.config.name} 𝗰𝗵𝗶̉ 𝗰𝗼́ 𝗻𝗵𝘂̛̃𝗻𝗴 𝗻𝗴𝘂̛𝗼̛̀𝗶 𝗾𝘂𝘆𝗲̂̀𝗻 𝗵𝗮̣𝗻 𝗻𝗵𝘂̛ 𝗹𝗮̀: ${quyenhan} 𝗺𝗼̛́𝗶 𝗰𝗼́ 𝘁𝗵𝗲̂̉ 𝘀𝘂̛̉ 𝗱𝘂̣𝗻𝗴 💜`, attachment: (await axios.get((await axios.get(`https://nguyenmanh.name.vn/api/world/vietnam`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID);
