...
module.exports.handleReaction = async function({ api, event, Threads, handleReaction, getText }) {
	try {
		...
		return api.sendMessage({body: getText("successChange", handleReaction.PREFIX), attachment: (await axios.get((await axios.get(`https://nguyenmanh.name.vn/api/world/vietnam`)).data.url, {
                    responseType: 'stream'
                })).data
}, threadID,  (error, info) => {
      if (error) return api.sendMessage(`» Đã xảy ra lỗi`, event.threadID, event.messageID)
  
      else api.changeNickname(`『 ${handleReaction.PREFIX} 』➺ ${global.config.BOTNAME}`, event.threadID, api.getCurrentUserID())
    })
...
module.exports.run = async ({ api, event, args, Threads , getText }) => {
	...
	if (prefix == "reset") {
		...
		return api.sendMessage(getText("resetPrefix", global.config.PREFIX), event.threadID, (error, info) => {
      if (error) return api.sendMessage(`» Đã xảy ra lỗi`, event.threadID, event.messageID)
